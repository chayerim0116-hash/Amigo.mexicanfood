# AGENTS.md — Premium Site Project

> Drop this file in the same folder as the PDF that came with it, then open that folder in Codex. Codex reads it automatically and uses it to bias every choice toward premium output.
>
...
When relevant, use these skills (available in the Codex marketplace):
...
3. They paste it into Codex with a natural sentence ("use this card for the 'How we work' section")
...
2. **Setup** — install Codex desktop, install design skills, add Motion as a dependency
...
Don't try to do all phases in one prompt. Each one is a separate conversation. Inside Phase 4, the "three rounds" pattern (structure first, motion second, polish last) gives Codex a chance to get the bones right before fancy details.
